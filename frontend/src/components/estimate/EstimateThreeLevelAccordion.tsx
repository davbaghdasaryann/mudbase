...
interface AccordionItem {
    ...
    /** For group rows: sum of all child works' unit prices (changableAveragePrice). */
    groupSumPrice?: number | null;
    /** For group rows: sum of all child works' unit prices including materials (when qty > 0). */
    groupSumUnitPrice?: number | null;
}
...
const EstimateThreeLevelNestedAccordion = forwardRef<EstimateThreeLevelNestedAccordionRef, EstimateThreeLevelNestedAccordionProps>(function EstimateThreeLevelNestedAccordion(props, ref) {
    ...
    const handleAddEmptyRow = async (targetId: string, isSubsection: boolean = false, isGroup: boolean = false) => {
        ...
        if (isSubsection) {
            ...
            // targetId is a section ID, need to find or use the empty subsection
            const section = sectionsRef.current.find((s) => s._id === targetId);
            if (section && section.children && section.children.length > 0) {
                subsectionIdForApi = section.children[0]._id;
            } else {
        ...
        try {
            ...
            console.error('Failed to create custom labor item:', error);
        }
    };
    ...
    const mapLevelItem = (item: any): AccordionItem => {
        ...
        if (item.groupSumPrice != null) itemArr.groupSumPrice = item.groupSumPrice;
        if (item.groupSumUnitPrice != null) itemArr.groupSumUnitPrice = item.groupSumUnitPrice;
            ...
            if (!hasNamedSubsection) {
                ...
                const emptySubsection: AccordionItem = {
                    ...
                    children: fetchedItems.map((item) => {
                        ...
                        if (item.groupSumPrice != null) newItem.groupSumPrice = item.groupSumPrice;
                        if (item.groupSumUnitPrice != null) newItem.groupSumUnitPrice = item.groupSumUnitPrice;
                        // }
                                                ...
                                                <Tooltip title={t('Import from Favorites')} arrow placement='top'>
                                                    <IconButton
                                                        ...
                                                        onClick={async (e) => {
                                                            ...
                                                            const subsectionId = String(res._id);
