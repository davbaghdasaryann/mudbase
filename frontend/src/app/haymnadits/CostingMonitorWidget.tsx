...
export default function CostingMonitorWidget({ estimateId, estimateName, onDelete }: Props) {
    ...
    return (
        <Box sx={{
            ...
            borderRadius: 3,
            border: isOver ? '1px solid rgba(198,40,40,0.3)' : '1px solid rgba(0,171,190,0.18)',
            boxShadow: '0 4px 24px rgba(0,171,190,0.08)',
            p: 2.5,
        }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', mb: 2 }}>
                <Box sx={{ minWidth: 0 }}>
                    <Typography sx={{ fontSize: '0.72rem', color: '#888', mb: 0.3 }}>Ծախսագրում monitoring</Typography>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <Box sx={{ display: 'flex', justifyContent: 'space-between', gap: 2 }}>
                        <Box>
                            <Typography sx={{ fontSize: '0.68rem', color: '#999', mb: 0.2 }}>Budget</Typography>
                            <Typography sx={{ fontSize: '0.92rem', fontWeight: 700, color: TEAL }}>
                                {formatCurrencyRounded(budget ?? 0)} AMD
                            </Typography>
                        </Box>
                        <Box sx={{ textAlign: 'right' }}>
                            <Typography sx={{ fontSize: '0.68rem', color: '#999', mb: 0.2 }}>Spent</Typography>
