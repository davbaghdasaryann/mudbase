...
export default function SubcontractorDialog({ open, onClose, onCostAdded }: Props) {
    ...
    const handleClose = () => {
        setWorkName(''); setContractor(''); setQty(''); setUnitPrice('');
        onClose();
    };
        ...
        <Dialog open={open} onClose={handleClose} maxWidth='xs' fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
            ...
            <DialogContent sx={{ pt: 1 }}>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1.2 }}>
                    <Field autoFocus label='Աշխատանք անվանումը' value={workName} onChange={setWorkName} />
                    <Field label='Կապալառու' value={contractor} onChange={setContractor} />
