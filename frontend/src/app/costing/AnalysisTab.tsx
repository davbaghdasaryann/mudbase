...
export default function AnalysisTab({ estimate, estimateSnapshot, unforeseenEstimate, unforeseenSnapshot, onDeleteUnforeseen, actualData, costHistory, pahestEntries }: Props) {
    ...
    const getActuals = (row: LaborRow) => {
        ...
        if (row.isGroupRow && row.childIds && row.childIds.length > 0) {
            // Group rows: sum children's actuals (mirrors how estimate derives group unit price from children)
            let childActTotal = 0;
            for (const childId of row.childIds) {
                ...
                const childVol = parseFloat((childA?.spent ?? '').replace(',', '.')) || 0;
                childActTotal += childVol + salTotal + matActTotal;
            }
            const actQty = row.quantity ?? 0;
            return { actQty, actTotal: childActTotal, hasData: childActTotal > 0 };
        }
