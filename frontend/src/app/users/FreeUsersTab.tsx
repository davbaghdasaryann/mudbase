...
export default function FreeUsersTab() {
    ...
    const onRemove = React.useCallback((freeUserId: string) => {
        confirmDialog(t('Are you sure?')).then((result) => {
            if (result.isConfirmed) {

                Api.requestSession<any>({ //TODO change any to interface
                    command: 'users/remove_free_user',
                    args: { freeUserId: freeUserId }
                })
                    .then(() => {
                        apiData.invalidate();
                    })
