...
interface LaborRow { _id: string; laborItemId: string; isGroupRow: boolean; fullCode: string; catalogName: string; laborOfferItemName: string; unitSymbol: string; quantity: number; changableAveragePrice: number; costingPrice?: number | null; libraryPrice?: number | null; cost: number; subsectionName: string; sectionName: string; }
...
{tab === 'works' && (
                    <Box sx={{ px: 2, pb: 2 }}>
                        ...
                        {!laborGroupsLoading && laborGroups.length > 0 && (() => {
                            const effectiveLaborItemCost = (item: any) => (laborOverrides[item._id] ?? laborOverrides[item.laborItemId] ?? item.costingPrice ?? item.libraryPrice ?? item.changableAveragePrice ?? 0) * (item.quantity ?? 0);
                                                    ...
                                                    {isOpen && group.items.map((item, idx2) => (
                                                        <TableRow key={String(item._id)} sx={{ '&:hover': { backgroundColor: '#f5fdfe' } }}>
                                                            ...
                                                            {editingPriceKey === 'li:' + item._id ? (
                                                                ...
                                                                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, justifyContent: 'flex-end' }}>
                                                                    <span style={{ fontSize: '0.82rem', color: '#777' }}>{Math.round((laborOverrides[item._id] ?? laborOverrides[item.laborItemId] ?? item.costingPrice ?? item.libraryPrice ?? item.changableAveragePrice) || 0) || '—'}</span>
                                                                    <Tooltip title={t('Edit')} placement='top' arrow><IconButton size='small' onClick={() => { setEditingPriceKey('li:' + item._id); setEditingPriceValue(String(laborOverrides[item._id] ?? laborOverrides[item.laborItemId] ?? item.costingPrice ?? item.libraryPrice ?? item.changableAveragePrice ?? '')); }} sx={{ opacity: 0, 'tr:hover &': { opacity: 1 }, transition: 'opacity 0.15s', p: '2px' }}><EditOutlinedIcon sx={{ fontSize: 14, color: '#aaa' }} /></IconButton></Tooltip>
                                                                </Box>
                                                            )}
                                                        </TableCell>
...
{tab === 'materials' && (
                    <Box sx={{ px: 2, pb: 2 }}>
                        ...
                        {!matGroupsLoading && matGroups.length > 0 && (() => {
                            const effectiveMatItemCost = (item: any) => (matOverrides[item._id] ?? matOverrides[item.materialItemId] ?? item.costingPrice ?? item.libraryPrice ?? item.changableAveragePrice ?? 0) * (item.quantity ?? 0);
                                    ...
                                    <TableBody>
                                        {(() => {
                                            ...
                                            const renderGroup = (group: GroupedByMaterial, indent?: boolean) => {
                                                                                                const isOpen = !!matGroupExpanded[group.materialItemId]; const matGroupDisplayCost = group.items.reduce((s: number, it: any) => s + effectiveMatItemCost(it), 0);
                                                return (
                                                        ...
                                                        {isOpen && group.items.map((item, idx2) => (
                                                            <TableRow key={String(item._id)} sx={{ '&:hover': { backgroundColor: '#f5fdfe' } }}>
                                                                ...
                                                                {editingPriceKey === 'mi:' + item._id ? (
                                                                    ...
                                                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, justifyContent: 'flex-end' }}>
                                                                        <span style={{ fontSize: '0.82rem', color: '#777' }}>{Math.round((matOverrides[item._id] ?? matOverrides[item.materialItemId] ?? item.costingPrice ?? item.libraryPrice ?? item.changableAveragePrice) || 0) || '\u2014'}</span>
                                                                        <Tooltip title={t('Edit')} placement='top' arrow><IconButton size='small' onClick={() => { setEditingPriceKey('mi:' + item._id); setEditingPriceValue(String(matOverrides[item._id] ?? matOverrides[item.materialItemId] ?? item.costingPrice ?? item.libraryPrice ?? item.changableAveragePrice ?? '')); }} sx={{ opacity: 0, 'tr:hover &': { opacity: 1 }, transition: 'opacity 0.15s', p: '2px' }}><EditOutlinedIcon sx={{ fontSize: 14, color: '#aaa' }} /></IconButton></Tooltip>
                                                                    </Box>
                                                                )}
                                                            </TableCell>
