                                ...
                                <Box>
                                    ...
                                    {!collapsedSections.has('quick') && <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 4 }}>
                                        {[
                                            ...
                                            { icon: <AddCardOutlinedIcon sx={{ fontSize: 24, color: '#e53935', opacity: 0.55 }} />, label: 'Այլ ծախսեր', onClick: () => {}, hoverBg: 'rgba(229,57,53,0.06)' },
                                    ...
                                    {!collapsedSections.has('costs') && (() => {
                                        const rows = detail?.rows ?? [];
                                        const fmtAMD = (n: number) => formatCurrencyRounded(Math.round(n)) + ' ֏';
                                        const sectionMap = new Map<string, { est: number; act: number }>();
                                        for (const r of rows) {
                                            const sec = r.sectionName || 'Other';
                                            const cur = sectionMap.get(sec) ?? { est: 0, act: 0 };
                                            cur.est += (r.estimatedUnitCost + (r.estimatedMaterialUnitCost ?? 0)) * r.quantity;
                                            const actUC = r.actualUnitCost ?? r.estimatedUnitCost;
                                            cur.act += actUC * r.quantity;
                                            sectionMap.set(sec, cur);
                                        }
                                        const sections = Array.from(sectionMap.entries()).filter(([, v]) => v.est > 0 || v.act > 0);
                                        const totalEstLabor = rows.reduce((s, r) => s + r.estimatedUnitCost * r.quantity, 0);
                                        const totalActLabor = rows.reduce((s, r) => s + ((r.actualLaborUnitCost ?? 0) * r.quantity), 0);
                                        const totalEstMat = rows.reduce((s, r) => s + (r.estimatedMaterialUnitCost ?? 0) * r.quantity, 0);
                                        const totalActMat = rows.reduce((s, r) => s + ((r.actualMaterialUnitCost ?? 0) * r.quantity), 0);
                                        const barItems = [
                                            { label: t('Labor'), est: totalEstLabor, act: totalActLabor, gi: 0 },
                                            { label: t('Materials'), est: totalEstMat, act: totalActMat, gi: 1 },
                                            ...sections.map(([sec, v], i) => ({ label: sec, est: v.est, act: v.act, gi: i + 2 })),
                                        ].filter(b => b.est > 0 || b.act > 0);
                                        ...
                                        return (
                                            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(3, 1fr)', md: 'repeat(4, 1fr)' }, gap: 2, mb: 2 }}>
                                                {barItems.map((item, idx) => {
                                                    const grad = OE_BAR_GRADS[item.gi % OE_BAR_GRADS.length];
                                                    const estId = `oe-est-${idx}`;
                                                    const actId = `oe-act-${idx}`;
                                                    const pctDiff = item.est > 0 && item.act > 0 ? ((item.act - item.est) / item.est) * 100 : null;
                                                    const chartData = [
                                                        { name: t('Estimated'), value: item.est, gradId: estId },
                                                        { name: t('Actual'), value: item.act, gradId: actId },
                                                    ].filter(d => d.value > 0);
                                                    return (
                                                        <Paper key={item.label} elevation={0} sx={{ border: '1px solid #d0f0f4', borderRadius: 3, p: 2, background: '#fff', boxSizing: 'border-box', display: 'flex', flexDirection: 'column', minHeight: 200, position: 'relative' }}>
                                                            {pctDiff !== null && (
                                                            ...
                                                            <Typography variant='caption' sx={{ fontWeight: 700, color: '#6b7280', textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: '0.68rem', textAlign: 'center', mb: 0.5, display: 'block' }}>{item.label}</Typography>
                                                            ...
                                                            <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 0.5 }}>
                                                                <Typography variant='caption' sx={{ color: '#555', fontSize: '0.68rem' }}>{oeFormatY(item.est)}</Typography>
                                                                {item.act > 0 && <Typography variant='caption' sx={{ color: OE_ACT_GRAD.top, fontWeight: 600, fontSize: '0.68rem' }}>{oeFormatY(item.act)}</Typography>}
                                                            </Box>
                                                        </Paper>
                                                    );
                                                })}
