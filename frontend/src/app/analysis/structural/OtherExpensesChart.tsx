...
const ACTUAL_GRAD = { top: '#FF8A65', bottom: '#E64A19', stroke: '#bf360c' };
        ...
        if (aylEntries) {
            ...
            if (total > 0) {
                const actualBar = {
                    ...
                    stroke: ACTUAL_GRAD.stroke,
                    isActual: true,
                    rawKey: 'actual',
                };
                const insertIdx = bars.findIndex(b =>
                    b.rawKey === 'smallScaleConstructionMaterials' ||
                    b.rawKey === 'smallScaleConstructionWork'
                );
                if (insertIdx >= 0) {
                    bars.splice(insertIdx + 1, 0, actualBar);
                } else {
                    bars.push(actualBar);
                }
            }
                ...
                <BarChart data={data} margin={{ top: 4, right: 12, left: 4, bottom: 0 }} barCategoryGap='30%'>
                    <defs>
                        ...
                        <linearGradient id='actual-grad' x1='0' y1='0' x2='0' y2='1'>
                            <stop offset='0%' stopColor={ACTUAL_GRAD.top} />
                            <stop offset='100%' stopColor={ACTUAL_GRAD.bottom} />
                        </linearGradient>
                    </defs>
